...
async function fullState(ctx, requested, sessionId) {
  ...
  const [board, memory, skills, read] = await Promise.all([
    readTasks(root),
    readMemory(root),
    readSkills(root),
    readRun(longloopDir(root), exists),
  ])
  // Every read path passes the §8.6 gate, including this one: the console is how a
  // human finds out what a restart left behind, so it must not be the one place
  // that reports a stale `armed` run as if it were still live.
  const run = await flushSuspension(ctx, longloopDir(root), suspendStaleRun(longloopDir(root), read))
...
async function suspendArmedRuns(ctx) {
  ...
  for (const workspace of knownWorkspaces(ctx)) {
    ...
    try {
      ...
      // The batch pass writes its own ledger fact below; the flag belongs to the
      // lazy gate only.
...
/**
 * When this process started. An `armed` run whose last write predates it was
 * alive in a *previous* process, which is the one fact §8.6/A6 needs.
 */
const PROCESS_STARTED_AT = Date.now()

/**
 * Suspend a run inherited from a dead process — synchronously, because the
 * driver's admission gate reads it.
 *
 * The mount-time pass (`suspendArmedRuns`) only sees workspaces the host already
 * knows, and at boot that list can be empty: a live restart left an `armed` run
 * sitting in a workspace the plugin had not been told about yet. So the same
 * decision is made lazily, the first time anything looks at the run — the driver
 * (which is where auto-resuming would actually happen) and every console/tool read.
 */
function suspendStaleRun(dir, run) {
  const current = run ?? readRunSync(dir)
  if (current === undefined || current.state !== 'armed' || current.updatedAt >= PROCESS_STARTED_AT) return current
  const next = {
    ...current,
    state: 'suspended',
    suspendedAt: Date.now(),
    suspendReason:
      current.inFlightRound === undefined
        ? '上一个进程结束时这条运行还是 armed；重新授权后才会继续'
        : `进程中断，第 ${current.inFlightRound} 轮（${roundIdFor(current, current.inFlightRound)}）结果未知；核对状态后再重新授权`,
    unknownOutcomeRoundId:
      current.inFlightRound === undefined ? current.unknownOutcomeRoundId : roundIdFor(current, current.inFlightRound),
    // The ledger fact is written by whoever can await it (see `flushSuspension`):
    // this function is the driver's synchronous gate, and a fire-and-forget write
    // here would race the next reader.
    suspendedFromProcess: true,
  }
  try {
    writeRunSync(dir, next)
  } catch {
    return current
  }
  return next
...
/**
 * Write the ledger fact for a suspension the synchronous gate just made.
 *
 * Awaited by every async read path, so "it is suspended" and "the ledger says
 * why" become true together rather than eventually.
 */
async function flushSuspension(ctx, dir, run) {
  if (run === undefined || run.suspendedFromProcess !== true) return run
  await appendLedger(dir, {
    kind: 'run-suspended',
    runId: run.id,
    round: run.round,
    detail: '进程中断（由读取路径发现：启动时的挂起扫描没覆盖到这个工作区）',
  })
  await patchRun(dir, exists, { suspendedFromProcess: undefined })
  if (ctx !== undefined) {
    emitLongloop(ctx, run, LONGLOOP_EVENT.run, runEventData({ ...run, state: 'suspended' }, { reason: 'suspended' }))
  }
  return run
}
...
async function handleRun(ctx, root, body) {
  ...
  const discovered = suspendStaleRun(dir)
  const run = await flushSuspension(ctx, dir, discovered === undefined ? await readRun(dir, exists) : discovered)
  if (run === undefined) return { ok: false, error: 'no run in this workspace' }
